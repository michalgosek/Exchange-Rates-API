...
func NewGlobalExchangeRatesQuery(currencies, base string) (GlobalExchangeRatesQuery, error) {
	if len(base) == 0 {
		return GlobalExchangeRatesQuery{}, fmt.Errorf("invalid base: must be a non empty string")
	}
	if len(currencies) == 0 {
		return GlobalExchangeRatesQuery{}, fmt.Errorf("invalid currencies: must be a non empty string")
	}
...
func NewCryptoExchangeRateQuery(from, to string, amount float64) (CryptoExchangeRateQuery, error) {
	if len(from) == 0 {
		return CryptoExchangeRateQuery{}, fmt.Errorf("invalid from: must be a non empty string")
	}
	if len(to) == 0 {
		return CryptoExchangeRateQuery{}, fmt.Errorf("invalid to: must be a non empty string")
	}
	if amount < 0 {
		return CryptoExchangeRateQuery{}, fmt.Errorf("invalid amount: must be a non negative number greater than zero")
	}
